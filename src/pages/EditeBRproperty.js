...
useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Append all formData fields except files
    for (let key in formData) {
      if (key !== 'feature_image' && key !== 'qr' && key !== 'images') {
        form.append(key, formData[key]);
      }
    }
  
    // Append feature_image if it exists
    if (formData.feature_image && formData.feature_image.length > 0) {
      form.append('feature_image', formData.feature_image[0]);
    }
  
    // Append qr if it exists
    if (formData.qr && formData.qr.length > 0) {
      form.append('qr', formData.qr[0]);
    }
    ...
    try {
      const response = await axios.put(`http://localhost:4000/api/update-rent/${id}`, form, {
        headers: {
  ...
  return (
    <div>
      ...
      <div className="pt-2">
        <img
          src={formData.qr} // Assuming formData.qr contains the URL of the old image
          alt="Old QR Code"
