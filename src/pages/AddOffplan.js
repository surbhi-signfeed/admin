...
const getCurrentDate = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post('http://localhost:4000/api/upload-offplan', data, {
        headers: {
      ...
      Swal.fire('Error', 'There was an error updating the data', 'error');      console.error('There was an error submitting the form!', error.response ? error.response.data : error.message);
    }
  };
